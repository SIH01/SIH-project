...
export default function GetHelp() {
  ...
  async function handleSubmit(event) {
    ...
    if (!form.locationText.trim() && !coords) { setError("Add a location or attach your current location."); return; }
  ...
  if (confirmation) {
    return <main className="help-shell"><section className="help-card help-confirmation"><div className="confirmation-mark">✓</div><h1>Your request #{confirmation} has been received</h1><p>An admin will review your request and follow up using the contact details you provided.</p><div className="confirmation-actions"><button className="btn btn-relief" onClick={() => { setConfirmation(null); setForm(EMPTY_FORM); setCoords(null); setPreviews([]); }}>Submit another request</button><Link className="btn btn-outline-ink" to="/">Return home</Link></div></section></main>;
  }
