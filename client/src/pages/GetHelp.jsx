...
export default function GetHelp() {
  ...
  async function handleSubmit(event) {
    ...
    if (form.type !== "missing_person" && !form.locationText.trim() && !coords) { setError("Add a location or attach your current location."); return; }
    ...
    <main className="help-shell">
      <section className="help-card">
        ...
        <form onSubmit={handleSubmit}>
          <fieldset className="help-fieldset"><legend>Type of help needed</legend><div className="help-type-grid" role="radiogroup" aria-label="Type of help needed">{HELP_TYPES.map(([value, label, icon]) => <label key={value} className={`help-type-tile ${form.type === value ? "selected" : ""}`}><input type="radio" name="help-type" value={value} checked={form.type === value} onChange={() => set("type", value)} /><span className="help-type-icon" aria-hidden="true">{icon}</span><span>{label}</span></label>)}</div></fieldset>
