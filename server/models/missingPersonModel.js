...
const SELECT_FIELDS = `
  ...
  last_known_location, date_last_seen, photo_url, description, additional_information,
...
async function getPublic() {
  const { rows } = await pool.query(
        `select id, person_name, age, last_known_location, date_last_seen,
          photo_url, description, additional_information, status, created_at
     from missing_person_reports
     where status <> 'Closed'
     order by date_last_seen desc, created_at desc`
  );
  return rows;
}
...
async function create(data) {
  ...
  const { rows } = await pool.query(
    ...
    [reporter_name, reporter_contact, disaster_id || null, person_name, age ?? null,
     last_known_location, date_last_seen, photo_url || null, description || null, additional_information || null]
  );
  return rows[0];
}
